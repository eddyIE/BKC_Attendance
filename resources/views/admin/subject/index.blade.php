...
@section('content')
    <div class="row form-group">
        <div class="col-md-3 offset-9 text-right">
            <a class="btn btn-primary" href="{{ route('subject.create') }}">Tạo mới</a>
        </div>
    </div>
    <table id="datatable" class="table align-middle table-bordered">
        <thead>
            <tr class="bg-dark">
                ...
                <th class="fs-5 border-0 text-white text-center">Thời lượng</th>
        ...
        @foreach ($data as $each)
            @if($each->status == 1)
                <tr class="clickable-row" onclick="window.location='{{ route('subject.show', $each->id) }}'">
                    <td class="text-center border-0">{{ $loop->index + 1 }}</td>
                    <td class="border-0">{{ $each->name }}</td>
                    <td class="text-center border-0">{{ $each->recommend_hours }}</td>
                    <td class="border-0 align-middle text-center">{{ date_format($each->created_at, 'd/m/Y') }}</td>
                    <td class="border-0 align-middle text-center">
                        <form action="{{ route('subject.destroy', $each->id) }}" method="post">
                            @method('DELETE')
                            @csrf
            ...
            @else
                <tr class="clickable-row table-active" onclick="window.location='{{ route('subject.show', $each->id) }}'">
                    <td class="text-center border-0">{{ $loop->index + 1 }}</td>
                    <td class="border-0">{{ $each->name }}</td>
                    <td class="text-center border-0">{{ $each->recommend_hours }}</td>
                    <td class="border-0 align-middle text-center">{{ date_format($each->created_at, 'd/m/Y') }}</td>
                    <td class="border-0 align-middle text-center">
                        <form action="{{ route('subject.restore', $each->id) }}" method="post">
                            @method('PATCH')
                            @csrf
